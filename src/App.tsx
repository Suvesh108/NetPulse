...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F8FAFC] modern-grid-bg text-[#0F172A] relative flex flex-col font-sans selection:bg-blue-500/10 selection:text-blue-600">
      ...
      <header className="w-full shrink-0 flex items-center justify-between px-4 sm:px-8 md:px-12 h-12 sm:h-13 bg-white/90 backdrop-blur-xl border-b border-slate-200/80 z-50 sticky top-0 transition-all">
        ...
        <div className="flex items-center bg-slate-100/90 p-0.5 rounded-lg border border-slate-200/80 shadow-inner">
          <button
            ...
            className={`px-2.5 py-0.5 rounded-md text-[11px] font-bold transition-all duration-200 cursor-pointer flex items-center gap-1 ${
              unit === 'Mbps'
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold transition-all duration-200 cursor-pointer ${
                  isActive 
                    ? 'bg-white text-slate-900 shadow-sm' 
      ...
      {/* Main Responsive Compact Content Arena */}
      <main className="flex-1 flex flex-col items-center justify-start p-2 sm:p-3 md:px-6 relative z-10 w-full max-w-7xl 2xl:max-w-[1600px] mx-auto pb-16 md:pb-4">
        {renderTabContent()}
      </main>
